...
export default function FinancePage() {
  ...
  const fetchPatients = async () => {
    ...
    try {
      ...
      console.error('Fetch finance patients error:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDone = async (an) => {
    ...
    try {
      await api.post(`/workflow/${an}/finance-done`)
                ...
                {loading ? (
                  ...
                  displayedHistory.map((p) => {
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">
                        {p.finance_done_date ? new Date(p.finance_done_date).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' }) : '-'} น.
                      </td>
                      <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">
                        {getWaitTime(p.sent_finance_date, p.finance_done_date)}
                      </td>
                      <td className="px-4 py-3 font-medium text-blue-600">{p.an}</td>
                      <td className="px-4 py-3 text-muted-foreground">{p.hn}</td>
                      <td className="px-4 py-3">
